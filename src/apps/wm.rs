...
pub fn window_manager_task() {
    ...
    unsafe {
        ...
        WINDOWS[0] = Some(Window { id: 0, x: 30, y: 40, w: 270, h: 180, title: title0, bg_color: 0xFF1F2335, active: true });
                    ...
                    unsafe {
                        ...
                        if c == '\x08' {
                            ...
                            if len >= 4 && &cmd[0..4] == b"help" {
                                ...
                                SHELL_OUTPUT[0..resp.len()].copy_from_slice(resp);
                                SHELL_OUTPUT_LEN = resp.len();
                                if let Some(ref mut win) = WINDOWS[1] { win.bg_color = 0xFF2C1C30; }
                            } else {
                    ...
                    unsafe {
                        for idx in 2..4 {
                            if WINDOWS[idx].is_none() {
                                WINDOWS[idx] = Some(Window { id: idx, x, y, w, h, title, bg_color, active: false });
                                found_idx = Some(idx);
                                break;
                            }
                        }
